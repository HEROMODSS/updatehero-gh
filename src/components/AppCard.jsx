...
export default function AppCard({ repo, search, refreshTick }) {
  ...
  const repoNameMatches = !search || repo.name.toLowerCase().includes(search);
  const versionMatches = search && !!versions && versions.some((v) => v.name.toLowerCase().includes(search));

  // A search term needs this card's versions to know if it matches — load them
  // in the background (once) whenever a search is active and repo name alone
  // doesn't already answer the question.
  useEffect(() => {
    if (search && versions === null && !loading) load();
  }, [search]);

  // Auto-expand once we know this card matches the current search.
  useEffect(() => {
    if (search && (repoNameMatches || versionMatches)) setExpanded(true);
  }, [search, versionMatches]);

  // Silent background refresh every 30s, only for cards already expanded.
  useEffect(() => {
    if (refreshTick && expanded) load();
  ...
  const sorted = useMemo(() => {
    ...
    // Newest added first, regardless of live/off status.
    return [...list].sort((a, b) => new Date(b.added_at || 0) - new Date(a.added_at || 0));
  ...
  // Once we know versions (or repo name matched instantly), hide non-matches.
  if (search && !repoNameMatches && versions !== null && !versionMatches) {
    return null;
  }
